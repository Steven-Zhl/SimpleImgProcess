...
def ShowMenu(backImage):
    ...
    alignstr = '%dx%d+%d+%d' % (800, 450, (width_screen - 800) / 2, (height_screen - 450) / 2)
    window.geometry(alignstr)
    window.resizable(width=False, height=False)  # 设置窗口的长宽均不可变

    # 往窗口上添加按钮
    area_function = Frame(window, width=200)
    Label(window, text="行到水穷处，坐看云起时。", font="楷体", fg='black').pack(anchor='ne', padx=5, pady=5)
    Button(area_function, text='创建九宫格图', width=18, command=lambda: ControlPanel.CreateNineGrid()).pack(side='top',
                                                                                                       pady=10)
    Button(area_function, text='拖拽以进行截图', width=18, command=lambda: ControlPanel.Capture()).pack(side='top', pady=10)
    ...
    Button(area_function, text='自适应优化图像', width=18, command=lambda: ControlPanel.AutoAttributeImg()).pack(side='top',
                                                                                                          pady=10)
    Button(area_function, text='退出程序', width=18, command=lambda: ControlPanel.Exit()).pack(side='top', pady=10)
    area_function.pack(padx=60, side='left')

    Label(window, width=450, height=450, image=CV2Tk(backImage, 450, 450)).pack(anchor='ne')
...
def CV2Tk(image_CV, width, height):
    ...
    image_convert = cv.cvtColor(image_convert, cv.COLOR_BGR2RGBA)  # 转换颜色从BGR到RGBA
    image_convert = Image.fromarray(image_convert)  # 将图像转换成Image对象
    image_convert = ImageTk.PhotoImage(image=image_convert)  # 将image对象转换为imageTK对象
    return image_convert


# 获取读取的文件路径
def getImagePath():
    path_image = filedialog.askopenfilename(title="请选择文件", filetypes=[('图像文件', ['*.jpg', '*.jpeg'])])
    return path_image
...
class WindowBase:
    def __init__(self, title):
        ...
        alignstr = '%dx%d+%d+%d' % (800, 450, (screenwidth - 800) / 2, (screenheight - 450) / 2)
    ...
    def addLabel(self, master, image_CV):
        ...
        [width_resize, height_resize] = self.calcResize(width_image, height_image, 400, 400)
...
class DragCapture(WindowBase):
    ...
    def UpdatePreview(self, image_CV, preview, width_pre, height_pre):
        ...
        self.image_temp = Img.capturePart(image_CV, [i * rate for i in boundary]).copy()
        [width, height] = self.calcResize(self.image_temp.shape[1], self.image_temp.shape[0], width_pre, height_pre)
...
class MosaicImage(WindowBase):
    ...
    def UpdatePreview(self, image_CV, boundary, mosaicStrength, preview):
        if boundary is not None:  # boundary is None说明是更新原图而非展示预览图
            self.image_temp = Img.addMosaic(image_CV=image_CV, boundary=boundary, strength=mosaicStrength)
        else:
            self.image_temp = image_CV.copy()
        width_resize, height_resize = self.calcResize(image_CV.shape[1], image_CV.shape[0], 400, 400)
...
class GridImage(WindowBase):
    ...
    def showWindow(self, imgCV_List):
        ...
        # 计算缩放宽度和高度
        [reWidth, reHeight] = self.calcResize(imgCV_List[0].shape[1], imgCV_List[0].shape[0], 400 // 3, 400 // 3)
...
class AttributeImage(WindowBase):
    ...
    def showWindow(self, image_CV):
        ...
        val_brightness = Scale(area_scale, from_=2, to=-1, resolution=0.1, width=4, length=150)
        val_brightness.pack(side='left')
        val_color = Scale(area_scale, from_=2, to=-1, resolution=0.1, width=5, length=150)
        val_color.pack(side='left')
        val_contrast = Scale(area_scale, from_=2, to=-1, resolution=0.1, width=4, length=150)
        val_contrast.pack(side='left')
        val_sharpness = Scale(area_scale, from_=2, to=-1, resolution=0.1, width=5, length=150)
        ...
        # 添加画布
        [resizeWidth, resizeHeight] = self.calcResize(image_CV.shape[1], image_CV.shape[0], 400, 400)
...
class MosaicImage_Auto(WindowBase):
    ...
    def ResetPreview(self, image_CV, preview, previewWidth, previewHeight):
        ...
        [width, height] = self.calcResize(image_CV.shape[1], image_CV.shape[0], previewWidth, previewHeight)
        imgTK = CV2Tk(image_CV, width, height)
        preview.configure(image=imgTK)
...
class AttributeImage_Auto(WindowBase):
    ...
    def UpdatePreview(self, image_CV, width, height, preview):
        ...
        self.tempImg = newPic
        [resizeWidth, resizeHeight] = self.calcResize(image_CV.shape[1], image_CV.shape[0], width, height)
