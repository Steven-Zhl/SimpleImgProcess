...
def save(image_CV, path_dir, quality, name):
    ...
    if isinstance(image_CV, list):
        for i in range(len(image_CV)):
            path_save = path_dir + '/' + (('Untitle_' if name == '' else (name + "_")) + str(i + 1)) + '.jpg'
            if quality == 'Origin':
        ...
        path_save = path_dir + '/' + ('Untitle' if name == '' else name) + '.jpg'
        if quality == 'Origin':
...
def capturePart(image_CV, boundary):
    ...
    x_right = image_CV.shape[1] - 1 if boundary[1] > image_CV.shape[1] - 1 else boundary[1]
    y_top = 0 if boundary[2] < 0 else boundary[2]
    y_bottom = image_CV.shape[0] - 1 if boundary[3] > image_CV.shape[0] - 1 else boundary[3]
...
def gridToNine(image_CV, blockNum_inRow):
    ...
    for i in range(0, blockNum_inRow):
        for j in range(0, blockNum_inRow):
            boundary = [j * width_block, (j + 1) * width_block, i * height_block, (i + 1) * height_block]
    ...
    if strength != 0 and strength != 1:  # 当strength==0或==1时，将视为不进行马赛克处理
        for i in range(0, mosaicPart_width // strength + 1):
            for j in range(0, mosaicPart_height // strength + 1):
                ...
                bottomEdge = min(boundary[2] + (j + 1) * strength, image_CV.shape[0] - 1)
                part_image = image_CV[topEdge:bottomEdge, leftEdge:rightEdge]  # 获取被打码部分的子图，用于计算颜色均值
                avg_color = np.average(np.average(part_image, axis=0), axis=0)  # 先计算每行的均值，再计算总的均值
...
def enhance(image_CV, strength):
    ...
    # 截取并将其转化为可调整的对象
    image_brightPart = Image.fromarray(cv.cvtColor(image_CV[0:height_block - 1, 0:width_block - 1], cv.COLOR_BGR2RGBA))
    ...
    # 调整并转化为OpenCV图片对象
    image_brightened = ImageEnhance.Brightness(image_brightPart).enhance(strength[0] + 1)  # 将Image对象转换为可调整对象
    image_brightened_CV = cv.cvtColor(np.asarray(image_brightened), cv.COLOR_RGB2BGR)  # 将其转换为OpenCV对象
    image_colored = ImageEnhance.Color(image_colorPart).enhance(strength[1] + 1)  # 调整亮度后的可调整对象
    image_colored_CV = cv.cvtColor(np.asarray(image_colored), cv.COLOR_RGB2BGR)  # 将其转换为OpenCV对象
    image_contrasted = ImageEnhance.Contrast(image_contrastPart).enhance(strength[2] + 1)  # 调整亮度后的可调整对象
    image_contrasted_CV = cv.cvtColor(np.asarray(image_contrasted), cv.COLOR_RGB2BGR)  # 将其转换为OpenCV对象
    image_sharped = ImageEnhance.Sharpness(image_sharpPart).enhance(strength[3] + 1)  # 调整亮度后的可调整对象
    image_sharped_CV = cv.cvtColor(np.asarray(image_sharped), cv.COLOR_RGB2BGR)  # 将其转换为OpenCV对象
    # 将图片按照对应位置赋值
    ...
    image_enhanced[0:height_block - 1, width_block - 1:2 * width_block - 2] = image_colored_CV
    image_enhanced[height_block - 1:2 * height_block - 2, 0:width_block - 1] = image_contrasted_CV
    image_enhanced[height_block - 1:2 * height_block - 2, width_block - 1:2 * width_block - 2] = image_sharped_CV
...
def locateFace(image_CV):
    ...
    location_face = face_cascade.detectMultiScale(grayImg, scaleFactor=1.2, minNeighbors=5, minSize=(5, 5))
